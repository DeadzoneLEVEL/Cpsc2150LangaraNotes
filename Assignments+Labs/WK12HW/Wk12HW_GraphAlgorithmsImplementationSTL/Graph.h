        ...
        while(!pq.empty()){
            ...
            for(const auto& neighbor:temp.first.neighbors){
                if(visited[neighbor.first]==false){
                    ...
                    // std::cout<<"        This "<<neighbor.first <<" is alt: "<<alt<<std::endl;
                    // auto v=distances.find(neighbor.first);
                    // auto v2=prev.find(neighbor.first);
                    // if(alt<v->second){
                    if(alt<distances[neighbor.first]){
                        // auto it=location[neighbor.first];
                        // std::cout<<locations[neighbor.first].name<<"\n";
